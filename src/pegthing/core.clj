...
(defn row-num
  "Returns the row number to which the position belongs to: pos 1 in row 1, posiions 2 and 3 in 2, etc"
  [pos]
...
(defn connect
  ...
  (if (in-bounds? max-pos neighbour destination)
    (reduce (fn [new-board [p1 p2]] (assoc-in new-board [p1 :connections p2] neighbour))
              board
...
(defn add-pos
  "Pegs the postion and performs connections"
  [board max-pos pos]
...
(defn ansi
  "Produce a string that will apply an ansi style"
  [style]
...
(defn prompt-move
  ...
  (println "\nHere's your board:")
  (print-board board)
  (println "Move frome where to where? Enter two letters:")
...
(defn prompt-rows
  ...
  (let [rows (read-string (get-input 5))
